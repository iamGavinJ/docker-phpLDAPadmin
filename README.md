...
Latest release: 0.11.0 - phpLDAPadmin 1.2.6.7 - [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/osixia/phpldapadmin/) 
...
- [osixia/phpldapadmin](#osixiaphpldapadmin)
  ...
  - [Beginner Guide](#beginner-guide)
    ...
    - [Use your own templates directory](#use-your-own-templates-directory)
    - [HTTPS](#https)
  ...
  - [Advanced User Guide](#advanced-user-guide)
    - [Extend osixia/phpldapadmin:0.11.0 image](#extend-osixiaphpldapadmin0100-image)
...
Run a phpLDAPadmin docker image by replacing `ldap.example.com` with your ldap host or IP :
    ...
    docker run -p 6443:443 \
            ...
            --detach osixia/phpldapadmin:0.11.0
...
Example script:
    ...
    docker run --name phpldapadmin-service --hostname phpldapadmin-service --link ldap-service:ldap-host --env PHPLDAPADMIN_LDAP_HOSTS=ldap-host --detach osixia/phpldapadmin:0.11.0
...
- Link your config file at run time to `/container/service/phpldapadmin/assets/config/config.php` :
            ...
            docker run --volume /data/my-config.php:/container/service/phpldapadmin/assets/config/config.php --detach osixia/phpldapadmin:0.11.0
...
- Link your templates directory at run time to /container/service/phpldapadmin/assets/templates
            ...
            docker run --volume /data/phpldapadmin/assets/templates:/container/service/phpldapadmin/assets/templates --detach osixia/phpldapadmin:0.11.0
...
By default HTTPS is enable, a certificate is created with the container hostname (it can be set by docker run --hostname option eg: phpldapadmin.my-company.com).
	...
	docker run --hostname phpldapadmin.my-company.com --detach osixia/phpldapadmin:0.11.0
...
Add --env PHPLDAPADMIN_HTTPS=false to the run command :
    ...
    docker run --env PHPLDAPADMIN_HTTPS=false --detach osixia/phpldapadmin:0.11.0
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] osixia/phpldapadmin:0.11.0 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach osixia/phpldapadmin:0.11.0 --loglevel debug

See all command line options:

	docker run osixia/phpldapadmin:0.11.0 --help
...
- **PHPLDAPADMIN_LDAP_HOSTS**: Set phpLDAPadmin server config. Defaults to :
  ...
  If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env PHPLDAPADMIN_LDAP_HOSTS="#PYTHON2BASH:[{'ldap.example.org': [{'server': [{'tls': True}]},{'login': [{'bind_id': 'cn=admin,dc=example,dc=org'}]}]}, 'ldap2.example.org', 'ldap3.example.org']" --detach osixia/phpldapadmin:0.11.0
...
### Extend osixia/phpldapadmin:0.11.0 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

    FROM osixia/phpldapadmin:0.11.0
